...
// Aba Glossário
const wsGlossario = utils.aoa_to_sheet(glossario);
wsGlossario['!cols'] = [{ wch: 30 }, { wch: 100 }];
utils.book_append_sheet(wb, wsGlossario, 'Glossário');

// Aba Regras de Negócio
const wsRegras = utils.aoa_to_sheet(regras);
wsRegras['!cols'] = [{ wch: 8 }, { wch: 100 }];
utils.book_append_sheet(wb, wsRegras, 'Regras de Negócio');

const outPath = '/home/kaue/Área de trabalho/Pós/Fase 1 /TechChallange/LinguagemUbiqua_GestaoClientes.xlsx';
writeFile(wb, outPath);
console.log('Excel criado em:', outPath);
