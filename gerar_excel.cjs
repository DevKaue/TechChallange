...
const wsG = XLSX.utils.aoa_to_sheet(glossario);
wsG['!cols'] = [{ wch: 32 }, { wch: 110 }];
XLSX.utils.book_append_sheet(wb, wsG, 'Glossário');

const wsR = XLSX.utils.aoa_to_sheet(regras);
wsR['!cols'] = [{ wch: 8 }, { wch: 110 }];
XLSX.utils.book_append_sheet(wb, wsR, 'Regras de Negócio');
